...
async function uploadBrandLogos() {
  ...
  if (!AZURE_STORAGE_ACCOUNT || !AZURE_STORAGE_ACCOUNT_KEY || !AZURE_STORAGE_CONTAINER) {
  ...
  try {
    ...
    const logosDir = path.join(process.cwd(), "public", "로고");
    
    for (const brand of brands) {
      const logoFileName = brandLogoMapping[brand.name as keyof typeof brandLogoMapping];
      ...
      try {
        ...
        await blockBlobClient.uploadFile(logoPath, {
          blobHTTPHeaders: {
...
// 제품명에서 용량 정보 제거 함수
function removeVolumeFromName(name: string): string {
  return name
...
function adjustPrice(originalPrice: number): number {
  if (originalPrice <= 10000) return originalPrice;
  
  // 10,000원을 초과하는 경우 적절히 조정
  if (originalPrice <= 20000) return Math.floor(originalPrice * 0.5); // 50% 할인
  if (originalPrice <= 30000) return Math.floor(originalPrice * 0.35); // 65% 할인  
  if (originalPrice <= 50000) return Math.floor(originalPrice * 0.25); // 75% 할인
...
async function main() {
  ...
  // 브랜드 로고 업로드 
  await uploadBrandLogos();

  // 카테고리 및 브랜드 ID 가져오기
  const categories = await prisma.category.findMany({
    select: { id: true, slug: true }
  });
  const categoryMap = new Map(categories.map((cat: { slug: string; id: number }) => [cat.slug, cat.id]));
  
  const brands = await prisma.brand.findMany({
    select: { id: true, name: true }
  });
  const brandMap = new Map(brands.map((brand: { name: string; id: number }) => [brand.name, brand.id]));
